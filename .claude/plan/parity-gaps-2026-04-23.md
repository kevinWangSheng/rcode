...
|---|---|---|---|---|
| 1 | cc-api | `cache_control` wire format never applied — prompt caching dead | TS api.ts:72–76, 297–320; Rust client.rs/request.rs | yes |
| 2 | cc-api | `thinking.type:"enabled"` + `budget_tokens` not settable on CreateMessageRequest | TS thinking.ts, sideQuery.ts:58/174; Rust request.rs | yes |
...
| 5 | cc-session | No interrupt-marker append path → resume can't detect pending tool | TS utils/messages.ts INTERRUPT_MESSAGE; Rust missing | yes |
| 6 | cc-session | FileHistorySnapshot entries not persisted → edited-file state lost on resume | TS utils/fileHistory.ts:39–52; Rust SessionMetadata 56–65 | yes |
