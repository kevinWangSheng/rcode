...
| 6 | cc-session | ⏳ PARTIAL (2026-04-23): types in `fix-session-resume-integrity`; relocated to `cc-core::file_history` and `Tool::execute` migrated to `(input, &ToolContext)` carrying `Arc<dyn SessionSink>` in `fix-tool-context-refactor`. Producer-side emission on Edit/Write success still pending — owns `fix-file-history-snapshot-producers` (now unblocked by tool-context + stream-mock-harness landings). | TS utils/fileHistory.ts:39–52; Rust `cc-core/src/file_history.rs` + `cc-core/src/tool.rs::SessionSink` | yes |
