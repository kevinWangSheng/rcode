...
| 6 | cc-session | ✅ DONE (2026-04-23): full chain landed. `fix-session-resume-integrity` shipped types; `fix-tool-context-refactor` plumbed `SessionSink` access through every tool; `fix-file-history-snapshot-producers` makes Edit / Write call `ctx.session.append_file_history_snapshot_for_path(...)` on successful mutation, persisting pre-edit bytes to a per-session sidecar (TS-shape `{session_dir}/file-history/<hash>@v{n}`) and emitting a `file-history-snapshot` JSONL entry that points at it. `fix-file-history-backup-versioning` (2026-04-23 QA follow-up) closes the repeat-edit collision: each snapshot gets its own `@v{n}` sidecar via a filesystem-scanned version counter, so two successful edits of the same file preserve distinct pre-images. Net-new files don't snapshot. 64 MiB warn threshold. | TS utils/fileHistory.ts:39–52; Rust `cc-tools/src/{edit,write,file_history}.rs` + `cc-session/src/lib.rs::{append_file_history_snapshot_for_path, next_backup_version}` | yes |
