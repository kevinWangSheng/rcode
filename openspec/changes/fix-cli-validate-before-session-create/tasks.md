...
- [x] 1.1 Add `struct ParsedCliOptions { thinking: Option<ThinkingConfig> }`
      near the other `Cli`-adjacent types in `rust/cc/src/main.rs`.
      Leave room for future fields.
- [x] 1.2 Add `fn parse_cli_options(cli: &Cli) -> Result<ParsedCliOptions,
...
- [x] 2.1 In `rust/cc/src/main.rs::run`, call `let parsed =
...
- [x] 2.2 Replace the existing inline `let thinking = match
...
- [x] 2.3 Add a comment above the new call site noting "validation
      goes first so failures don't leak session files".

## 3. Tests

- [x] 3.1 Add `parse_cli_options_rejects_invalid_thinking` in
      `cc/src/main.rs::tests` per the proposal §3 snippet. Asserts
      `Err` with a message containing "--thinking".
- [x] 3.2 Add `parse_cli_options_accepts_valid_inputs` covering
...
- [~] 3.3 Optional: end-to-end smoke shell test in
      ...
      *Deferred: `cc-session` hard-codes `dirs::home_dir()` with no
      `CLAUDE_HOME` override. Covered by the §4.4 manual smoke
      (22 → 22 file count, no Session: line printed).*

## 4. Verification

- [x] 4.1 `cargo fmt -p claude-cli` clean.
- [x] 4.2 `cargo clippy -p claude-cli --all-targets -- -D warnings`
      clean.
- [x] 4.3 `cargo test -p claude-cli` — all green, including new
      tests.
- [x] 4.4 Manual smoke: count `*.jsonl` under
...
- [x] 5.1 Commit references the smoke pass that surfaced the gap
