# cli-startup-order Specification

## Purpose

Pins the ordering of work inside `claude::run`: pure post-clap
validation MUST run before any disk side effect (session JSONL
creation, credential refresh, settings load) so a user typo leaves
no orphan files and no misleading `Session: <uuid>` stderr line.
