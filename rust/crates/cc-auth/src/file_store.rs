...
/// Read the full OAuth token bundle (access + refresh + expires_at) from a
...
/// Read the full OAuth token bundle from the resolved default/override path.
/// Returns the path alongside the tokens so callers can write back to the
/// same location after a refresh.
pub fn read_oauth_tokens_from_file() -> CcResult<Option<(PathBuf, OAuthTokens)>> {
    let Some(path) = credentials_file_path() else {
        return Ok(None);
    };
    let Some(tokens) = read_oauth_tokens_from_path(&path)? else {
        return Ok(None);
    };
    Ok(Some((path, tokens)))
}
...
mod tests {
    ...
    fn read_oauth_tokens_returns_full_bundle() {
        ...
        assert_eq!(tokens.expires_at, Some(9_000_000));
    }

    #[test]
    fn read_oauth_tokens_returns_none_when_missing() {
        let dir = tempfile::tempdir().unwrap();
        let tokens = read_oauth_tokens_from_path(&dir.path().join("nope.json")).unwrap();
